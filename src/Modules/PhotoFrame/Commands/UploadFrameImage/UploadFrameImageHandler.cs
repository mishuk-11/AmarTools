...
internal sealed class UploadFrameImageHandler
    : IRequestHandler<UploadFrameImageCommand, Result<PhotoFrameSetupDto>>
{
    ...
    public UploadFrameImageHandler(
        AppDbContext db, ICurrentUserService currentUser,
        IUnitOfWork uow, IFileStorageService storage, IImageService imageService)
    {
        _db           = db;
    ...
    public async Task<Result<PhotoFrameSetupDto>> Handle(
        UploadFrameImageCommand command, CancellationToken ct)
    {
        ...
        if (!_imageService.IsValidUpload(command.ImageStream, command.ContentType))
            return Error.Validation("PhotoFrame.InvalidFile",
                "Only JPG and PNG files up to 10 MB are accepted as frame images.");
        ...
        // ── Delete old frame if one exists ────────────────────────────────────
        if (!string.IsNullOrWhiteSpace(config.FrameImagePath))
            await _storage.DeleteAsync(config.FrameImagePath, ct);

        // ── Save new frame ────────────────────────────────────────────────────
        command.ImageStream.Position = 0;
        var storagePath = await _storage.SaveAsync(
            command.ImageStream, command.FileName, "frames", ct);

        config.SetFrameImage(storagePath);
